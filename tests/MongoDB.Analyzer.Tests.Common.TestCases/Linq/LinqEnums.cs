...
namespace MongoDB.Analyzer.Tests.Common.TestCases.Linq
{
    public sealed class LinqEnums : TestCasesBase
    {
        ...
        [MQL("Aggregate([{ \"$match\" : { \"$or\" : [{ \"EnumUInt64\" : 0 }, { \"EnumUInt64\" : 999 }, { \"EnumUInt64\" : -1 }] } }])")]
        ...
        [MQL("Aggregate([{ \"$match\" : { \"$or\" : [{ \"EnumInt64\" : 0 }, { \"EnumInt64\" : 999 }, { \"EnumInt64\" : 9223372036854775807 }] } }])")]
        ...
        [MQL("Aggregate([{ \"$match\" : { \"$or\" : [{ \"Vehicle.VehicleType.Type\" : 0 }, { \"Vehicle.VehicleType.Type\" : 2 }] } }, { \"$project\" : { \"_v\" : \"$Vehicle.LicenceNumber\", \"_id\" : 0 } }])")]
        public void Enum_not_typed_in_nested_class()
        {
            _ = GetMongoQueryable<Person>()
                .Where(u => u.Vehicle.VehicleType.Type == VehicleTypeEnum.Bus || u.Vehicle.VehicleType.Type == VehicleTypeEnum.Motorcycle)
                .Select(u => u.Vehicle.LicenceNumber);
        }

        [MQL("Aggregate([{ \"$match\" : { \"MPG\" : { \"$gt\" : 20.0 } } }, { \"$project\" : { \"_v\" : [\"$VehicleMake\", \"$Type\"], \"_id\" : 0 } }])")]
        ...
        public void Enum_in_type_argument_2()
        {
            _ = GetMongoCollection<VehicleType>().AsQueryable()
                .Where(v => v.MPG > 20)
                .Select(v => new System.Collections.Generic.List<VehicleTypeEnum> { VehicleTypeEnum.Bus, VehicleTypeEnum.Car, VehicleTypeEnum.Motorcycle });
        }
        ...
        public void Enum_in_type_argument_query_syntax_2()
        {
            _ = from vehicleType in GetMongoQueryable<VehicleType>()
                where vehicleType.MPG > 20
                select new System.Collections.Generic.List<VehicleTypeEnum> { VehicleTypeEnum.Bus, VehicleTypeEnum.Car, VehicleTypeEnum.Motorcycle };
        }
        ...
        [MQL("Aggregate([{ \"$match\" : { \"MPG\" : { \"$gt\" : 20.0 } } }, { \"$project\" : { \"_v\" : [\"$VehicleMake\", [\"$Type\", \"$Type\"]], \"_id\" : 0 } }])")]
        ...
        public void Query_Syntax()
        {
            ...
            _ = from person in GetMongoQueryable<Person>()
                where person.Vehicle.VehicleType.Type == VehicleTypeEnum.Bus || person.Vehicle.VehicleType.Type == VehicleTypeEnum.Motorcycle
